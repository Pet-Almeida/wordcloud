...
def cloud(image, text, max_word, max_font, random, stp):
...
def main():
    ...
    st.write("[Adaptado por Peterson Almeida](https://www.linkedin.com/in/pe7/)")
    ...
    stp = st.text_area("Adicione palavras para remover separadas com: ,")
    ...
    if image and text is not None:
        if st.button("Plot"):
            ...
            st.write("### Word cloud")
            st.write(cloud(image, text, max_word, max_font, random, stp), use_column_width=True)
